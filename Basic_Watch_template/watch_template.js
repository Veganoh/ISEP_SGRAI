...
export default class Watch extends THREE.Group {
    constructor(cityName, center = new THREE.Vector2(0.0, 0.0), radius = 0.75, nameBackgroundColor = 0xffffff, nameForegroundColor = 0x000000, dialColor = 0x000000, markersColor = 0xffffff, handsHMColor = 0xffffff, handSColor = 0xff0000) {
        ...
        */
        ...
        points.push(new THREE.Vector2(0.5 * radius,0.0));
        geometry = new THREE.BufferGeometry().setFromPoints(points);
        material = new THREE.LineBasicMaterial(handsHMColor);
        this.handH = new THREE.Line(geometry,material);
        this.add(this.handH);
        ...
        points = [];
        points.push(new THREE.Vector2(0.0,0.0));
        points.push(new THREE.Vector2(0.7 * radius,0.0));
        geometry = new THREE.BufferGeometry().setFromPoints(points);
        this.handM = new THREE.Line(geometry,material);
        this.add(this.handM); 
    ...
    update() {
        ...
        angle = Math.PI / 2.0 - (2.0 * Math.PI * time[0]  / 12.0) - (2.0 * Math.PI * time[1] / (12.0 * 60.0)) - (2.0 * Math.PI * time[2] / (12.0 * 3600.0));
